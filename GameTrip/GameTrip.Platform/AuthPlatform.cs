...
public class AuthPlatform : IAuthPlatform
{
    ...
    public async Task<IdentityResult> ConfirmEmailAsync(GameTripUser user, string token) => await _userManager.ConfirmEmailAsync(user, token);
    ...
    public async Task<IdentityResult?> ResetPasswordAsync(GameTripUser user, string password, string token) => await _userManager.ResetPasswordAsync(user, token, password);
